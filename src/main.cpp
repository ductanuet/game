...
bool LoadMedia()
{
    ...
    gMenu = Mix_LoadMUS("sound_menu.wav");
    if(gMenu == NULL)
    {
        printf( "Failed to load menu music! SDL_mixer Error: %s\n", Mix_GetError() );
		success = false;
    }
    gWin = Mix_LoadMUS("win_sound.wav");
    if(gWin == NULL)
    {
        printf( "Failed to load win music! SDL_mixer Error: %s\n", Mix_GetError() );
		success = false;
    }
    gLose = Mix_LoadMUS("game_over.wav");
    if (gLose == NULL)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            if(!game_over.LoadImg("game_over1.png", gscreen) )
            {
...
void menu()
{
    ...
    menuground.LoadImg("menu.png", gscreen);
